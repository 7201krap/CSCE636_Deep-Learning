...
class Config(object):
  ...
  max_epochs = 1000 # the number of max epoch
  early_stopping = 2
  dropout = 0.1
  lr = 0.001
  vocab_size= 0
...
class RNNLM_Model(nn.Module):
  ...
  def add_embedding(self, input_x):
    ...
    """
    ### YOUR CODE HERE
    input_x = [self.L(input_x[:, x]) for x in range(self.config.num_steps)]
    ### END YOUR CODE
    return input_x
  ...
  def add_model(self, input_x, initial_state):
    ...
    for i in input_x:
        h_to_h = torch.matmul(previous_state, self.H)
        i_to_h = torch.matmul(i, self.I)
        previous_state = self.sigmoid(h_to_h + i_to_h + self.b1)
        rnn_outputs.append(self.output_drop(previous_state))
    
    ### END YOUR CODE
    return rnn_outputs, previous_state 
...
def compute_loss(outputs, y, criterion):
  ...
  ### YOUR CODE HERE
  y = y.t().flatten()
  output_concat = torch.cat(outputs, 0)
#   print("shape of y:", y.shape).  640
#   print("shape of output_concat:", output_concat.shape).  # 640 x 10000 
...
def test_RNNLM():
  ...
  for epoch in range(config.max_epochs):
    ...
    if valid_pp < best_val_pp:
      ...
      torch.save(state, '/content/drive/MyDrive/Colab Notebooks/HW4/RNN/' + './ckpt.pth')
    if epoch - best_val_epoch > config.early_stopping:
      break
    print('Total time: {}'.format(time.time() - start))
    
    # my code to track perplexity for training and validation
    with open('train_pp.txt', 'a') as writefile:
        writefile.write(f'{epoch}, {train_pp}\n')
    with open('valid_pp.txt', 'a') as writefile:
        writefile.write(f'{epoch}, {valid_pp}\n')
    
  ## After training, we load the model and test it.  
  print("Testing")
  checkpoint = torch.load('/content/drive/MyDrive/Colab Notebooks/HW4/RNN/' + './ckpt.pth')
